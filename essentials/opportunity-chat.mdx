...
1. Find an opportunity of interest via Sweetspot's search functionality, then click the `Ask AI` button. This will redirect you to a chat interface.
...
5. **View Reasoning:** Sweetspot AI thinks before it responds, ensuring you get accurate information. Hover over the knowledge icon to view the reasoning provided for each message in your chat thread.
...
- **Keyword Search**: Use `cmd + f` to quickly find keywords within the currently opened solicitation file. Or, click on the magnifying glass on the left-hand side of the file viewer toolbar (located at the bottom of the page) to toggle the search interface.
- **Zoom Functionality**: Adjust the view by zooming in with `+` and out with `-` in the file viewer. This is located on the right-hand side of the file viewer toolbar.
...
- **File Management**: Navigate through the solicitation files by clicking on the file selector dropdown at the top of the file viewer.
- **Tables**: Organize information into digestable tables by prompting the chatbox with a query such as 'in a table' or 'tabular form'. Scroll to the right of the table to reveal a copy button, allowing you to copy and paste the table into any document editor.
